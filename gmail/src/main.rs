...
lazy_static! {
	...
	static ref GOOGLE_APP_REDIRECT_URL: String = env::var("GOOGLE_APP_REDIRECT_URL").expect("Env variable GOOGLE_APP_REDIRECT_URL not set");
	static ref GOOGLE_APP_CLIENT_ID: String = env::var("GOOGLE_APP_CLIENT_ID").expect("Env variable GOOGLE_APP_CLIENT_ID not set");
	static ref GOOGLE_APP_CLIENT_SECRET: String = env::var("GOOGLE_APP_CLIENT_SECRET").expect("Env variable GOOGLE_APP_CLIENT_SECRET not set");
...
async fn get_google_email(id_token: &str) -> Result<GoogleEmail, HttpResponse> {
	let response = new_http_client().get(format!("https://oauth2.googleapis.com/tokeninfo?id_token={}", id_token))
...
async fn get_access_token(code: &str) -> Result<AccessTokenBody, HttpResponse> {
	...
	let response = new_http_client().post("https://oauth2.googleapis.com/token")
		.send_form(&params)
		.await;
	match response {
		Ok(mut r) => {
...
async fn post_msg(msg_body: web::Json<PostBody>) -> HttpResponse {
	...
	let response = new_http_client().post("https://gmail.googleapis.com/gmail/v1/users/me/messages/send")
